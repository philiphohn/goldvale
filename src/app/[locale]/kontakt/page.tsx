...
export default async function KontaktPage({params}: {params?: Promise<{locale?: string}>}) {
  ...
  return (
    <section className="bg-[var(--color-background-2)] border-b border-[var(--color-line)] pt-24 pb-12 min-h-screen" id="kontakt">
      <div className="wrap">
        <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-[clamp(2rem,5vw,5rem)] items-start py-[clamp(4rem,8vw,6rem)]">
          <Reveal>
            <h1 className="font-medium text-[clamp(2rem,4.5vw,3.6rem)] tracking-[-0.025em] lowercase leading-[1.02]">
              {t('title_1')}
